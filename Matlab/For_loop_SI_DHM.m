...
h1 = function_for_loop.holo_read(strcat(path_1, name1)); % Read first hologram
h2 = function_for_loop.holo_read(strcat(path_1, name2)); % Read second hologram

% Display the first hologram
figure,imagesc(h1),colormap(gray),colorbar,title('Hologram h1'),daspect([1 1 1])

% output
out = function_for_loop.SIDHM(h1, h2, lambda, dxy, region, step);
